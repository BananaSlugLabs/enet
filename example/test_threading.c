...
void* runthread (void* r) {
    int retcode;
    int quit = 0;
    fprintf(stderr, "Server thread starting.\n");
    for (;!quit;)
    {
        ...
        if (retcode > 0)
        {
            ...
            switch (event.type)
            {
                ...
                case ENET_EVENT_TYPE_DISCONNECT:
                    ...
                    free(client);
                    break;
                case ENET_EVENT_TYPE_INTERRUPT:
                    fprintf(stderr, "User interrupted. Exiting.\n");
                    quit = 1;
                    break;
                default:
...
int main(int argc, char **argv)
{
    ...
    fprintf(stdout, "User requested terminate.\n");

    enet_host_interrupt(server);
